...
/** The FiscalPacket implementation used by EpsonFiscalDevice and BasicFiscalDevice. */
/**
 * @author Bosso Armando
 * @date 03/05/2013
 */

public class EpsonFiscalPacket extends AbstractFiscalPacket 
{
	private static final long serialVersionUID = 3113281446232338907L;
    private static final byte FLD = 0x1C; //separador de campos
	...
	public EpsonFiscalPacket(String encoding, int baseRolloverYear, FiscalPrinterDevice fiscalPrinter)
	{
		super(fiscalPrinter);
	...
	public byte[] encodeBytes() {
		...
		for (int i = 0, s = getSize(); i < s; i++) {
			...
			if(i != s - 1)
				result = ArrayUtils.append(result, FLD);
		}
	...
	public void decode(byte[] packetBytes) {
		...
		for(int i = 0; i < packetBytes.length; i++) {
			...
			if (b == FLD || i == (packetBytes.length - 1)) {
